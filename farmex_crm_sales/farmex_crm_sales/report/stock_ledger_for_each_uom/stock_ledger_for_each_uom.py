...
def execute(filters=None):
	...
	# Query the UOM Conversion Detail table to get UOMs for the item codes
	...
	# Build a dictionary to store UOMs and conversion factors for each item code
	item_uom_map = {}
	max_uoms = 0

	for detail in uom_details:
		item_code = detail['item_code']
		uom = detail['uom']
		conversion_factor = detail['conversion_factor']
		
		if item_code not in item_uom_map:
			item_uom_map[item_code] = {}
		item_uom_map[item_code][uom] = conversion_factor

	# Find the maximum number of UOMs for any item
	max_uoms = max(len(uoms) for uoms in item_uom_map.values())

	# Extend columns dynamically based on the maximum number of UOMs
	for uom in range(max_uoms):
		columns.extend(
			[
				{
					"label": _("UOM"),
					"fieldname": "uom_{}".format(uom + 1),
					"fieldtype": "Link",
					"options": 'UOM',
					"width": 90,
				},
	...
	# Process data and map UOMs and quantities
	for d in data:
		uoms = item_uom_map.get(d['item_code'], {})
		for idx, (uom, conversion_factor) in enumerate(uoms.items(), start=1):
			d[f"uom_{idx}"] = uom
			d[f"qty_{idx}"] = d['in_qty'] / conversion_factor

		# frappe.msgprint(str(d))

	
	data = [row for row in data if row.get("qty_1") != 0]
	return columns, data
