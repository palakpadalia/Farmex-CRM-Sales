...
frappe.query_reports["Stock Ledger for each UOM"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		} else if (column.label == "Qty" ) {
			let plain_value = $("<div>").html(value).text().trim();
			if (plain_value == 0) { value = "" }
            value = "<span style='color:green'>" + value + "</span>";
		// } else if (column.label == "UOM" ) {
        //     value = "<span style='color:green'>" + value + "</span>";
		}
		return value;
	},
